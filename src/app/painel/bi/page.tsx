...
export default async function BiPage({
  ...
  const [pedidos, gastos] = await Promise.all([
    prisma.order.findMany({
      ...
      select: {
        status: true, amountCents: true, netCents: true, shippingCents: true,
  ...
  // O frete sai da receita e do líquido: ele é dinheiro que entra pra
  // ser repassado à transportadora, não faturamento de produto.
  // Somado, inflaria o ROAS e o lucro — e é justamente essa conta que
  // decide quanto se investe em anúncio.
  const freteCents = pagos.reduce((a, p) => a + p.shippingCents, 0);
  const receitaCents = pagos.reduce((a, p) => a + (p.amountCents - p.shippingCents), 0);
  const liquidoCents = pagos.reduce(
    (a, p) => a + (p.netCents ?? p.amountCents) - p.shippingCents,
    0,
  );
  ...
  for (const p of pagos) {
    ...
    if (dia[k]) dia[k].receita += p.amountCents - p.shippingCents;
  }
  ...
  for (const p of pagos) {
    ...
    porProduto[k].receita += p.amountCents - p.shippingCents;
    porProduto[k].qtd += 1;
  }
  ...
  for (const p of pedidos) {
    ...
    if (p.status === "PAID") { porOrigem[k].pagos += 1; porOrigem[k].receita += p.amountCents - p.shippingCents; }
  }
  ...
  const valores = pagos.map((p) => p.amountCents - p.shippingCents);
  ...
  return (
    <PainelShell email={session.email}>
      <BiClient
        ...
        lojaId={lojaId}
        resumo={{
          receitaCents, liquidoCents, gastoCents, freteCents,
